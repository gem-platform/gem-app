...
##connect('gem', host='mongo', port=27017)
...
async def read_own_items(current_user: User = Depends(get_current_active_user)):
    return UserDb.objects[0].to_mongo()
    # return [{"item_id": "Foo", "owner": current_user.username}]

# sio = socketio.AsyncServer(async_mode='asgi')
# app2 = socketio.ASGIApp(sio)
# subapi = FastAPI(openapi_prefix="/socket.io")
# app.mount("/socket.io", subapi)


@app.post("/debug/wipeout")
...
# @app.post("/debug/createuser")
# async def create_user(name: str):
#     user = {
#         'username': name,
#         'full_name': name,
#         'email': name + '@example.com',
#         'hashed_password': pwd_context.get_password_hash(name),
#         'disabled': False,
#     }
